# -*- coding: utf-8 -*-
"""
Created on Fri Feb  5 11:50:06 2021
...
#Importation
        if self.fileformat == 0: # Importation procedure for Picasso hdf5 files.
            ...
            xloc = dataset['x'] * self.pxsize
            yloc = dataset['y'] * self.pxsize
            sxloc = dataset['lpx'] * self.pxsize
            syloc = dataset['lpy'] * self.pxsize
...
#                bg = full_list(:,5);
        ...
        Ntimes_min_div = int((self.listLocalizations[:,0].size/min_div))
        truefalse_sum_roi_acum = []
        self.listLocalizations_filtered = np.zeros((int(min_div*Ntimes_min_div),listLocalizations[1,:].size))
        ...
        for N in range(0, Ntimes_min_div+1):
            ...
            if self.rz_xyz == 3 or self.rz_xyz == 4:
        ...
        if self.rz_xyz == 1 or self.rz_xyz == 2 or self.rz_xyz == 3:
            z1 = (np.log(self.alphaF*self.N0)-np.log(photons1-(1-self.alphaF)*self.N0))/(1/self.dF)
            z1 = np.real(z1)
            ...
            if rz_xyz == 2:
                self.x = x_idx.flatten()
                self.y = y_idx.flatten()
                z = z1.flatten()
                ...
                self.simpler_output = np.column_stack((x_idx, y_idx, proyec_r, z1, photons_idx, frame_idx))
                print('SIMPLER done')
        # N0 Calibration
        ...
        if self.rz_xyz == 4:
            ...
            hist, bin_edges = np.histogram(photons_idx[c], bins = 20, density = False)
